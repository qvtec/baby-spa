  ...
  methods: {
    enable () {
      ...
      this.$axios.post('user/two-factor-authentication')
        .then(() => {
          this.$axios.get('user/two-factor-qr-code')
            .then(response => {
              this.qrCode = response.data.svg
              this.codeLoading = false
            })

          this.$axios.get('user/two-factor-recovery-codes')
            .then(response => {
    ...
    disable () {
      this.loading = true
      this.$axios.delete('user/two-factor-authentication')
        .then(() => {
    ...
    confirm () {
      this.$axios.post('user/confirm-password', this.form)
        .then(() => {
